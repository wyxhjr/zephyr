...
LOG_MODULE_REGISTER(imsic_demo_simple, LOG_LEVEL_INF);
...
static void demo_basic_operations(void)
{
	...
	LOG_INF("📊 Device Info:");
	LOG_INF("   - Device Name: %s", imsic_dev->name);
	LOG_INF("   - Device Ready: %s", device_is_ready(imsic_dev) ? "YES" : "NO");
...
/* Demo step 4: MSI (Message Signaled Interrupt) test */
static void demo_msi_test(void)
{
	LOG_INF("=== Step 4: MSI (Message Signaled Interrupt) Test ===");
...
/* Demo step 5: Performance and stress test */
static void demo_performance_test(void)
{
	LOG_INF("=== Step 5: Performance and Stress Test ===");
...
int main(void)
{
	LOG_INF("=== RISC-V IMSIC Demo Application (Simple) ===");
